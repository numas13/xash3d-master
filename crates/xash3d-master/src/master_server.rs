...
where
    A: ToSocketAddrs,
{
    for i in lookup_host(addr).await? {
...
impl Master {
    ...
    pub async fn run(&mut self, stop: Option<watch::Receiver<()>>) -> Result<(), Error> {
        match self {
...
impl<Addr: AddrExt> MasterServer<Addr> {
    ...
    pub async fn run(&mut self, mut stop: Option<watch::Receiver<()>>) -> Result<(), Error> {
        let mut buf = [0; 2048];
        loop {
            let read = self.sock.recv_from(&mut buf);
